...
Instead of using a system of linear difference equations, we can use a higher-order homogeneous linear difference equation. From \eqref{eq:linear-difference-model-system}, we can use the expression of $M_{n+1}$ and substitute it into the expression of $R_{n+1}$, giving us:
...
To create this model, we will start with \sysref{eq:linear-difference-model-system} and convert that to a system of ordinary differential equations. We will start by converting the first equation in \sysref{eq:linear-difference-model-system}. We can rewrite that equation in the following form:
...
We will convert the second equation in \sysref{eq:linear-difference-model-system} in a similar manner. We will first subtract $M_n$ on both sides:
...
We can rewrite \sysref{eq:nonlinear-difference-model-system} as one, higher order, nonlinear difference equation.
